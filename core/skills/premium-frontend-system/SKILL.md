...
When implementing scroll-driven animations, strictly adhere to these 5 Premium Fluid Motion Architecture Rules to prevent micro-hitches, scrub lag, and floaty disconnects.

#### 1. Spatial Compression Rule
Never attach independent ScrollTriggers to items that sit closer than 150px vertically. Instead, trigger the parent container and drive item entrances via a GSAP timeline with a 150–200ms stagger.

#### 2. Zero Debounce on Reveals
Avoid `ScrollTrigger.batch()` for content reveals; its 100ms debounce creates visible pop-in lag. Use direct triggers starting at `top 96%` so entrances begin before entering the focal area.

#### 3. No Mid-Scroll DOM Mutations
Never remove attributes (`removeAttribute`) or inject classes during an active scroll tween (`onStart`). Mark completion exclusively in `onComplete` or via pure CSS `:not(.revealed)`.

#### 4. Compositor Isolation
Ensure static typography (`p`, `h1-h6`, `li`) does NOT have `transition: color` or `transition: all`. Transitions belong strictly on `:hover` and `:focus`.

#### 5. Scroll Inertia Floor
When pairing Lenis with GSAP, use `duration: 1.0` (not >1.2s to prevent floaty lag), bind with `lenis.on('scroll', ScrollTrigger.update)`, and call `gsap.ticker.lagSmoothing(0)`.
