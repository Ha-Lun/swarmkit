...
> **opencode note:** This source is **disabled** in this opencode integration. `~/.claude/history.jsonl` is a Claude-Code-specific file that does not exist in opencode, so this source is skipped (see SKILL.md). The steps below are kept for reference only — do not attempt to read the history file.
