...
## Bash Date Arithmetic (Linux `date -d`)
...
START=$(date -d "-${N} days" +%Y-%m-%d)
END=$(date +%Y-%m-%d)

# this_week (Monday)
DOW=$(date +%u)  # 1=Mon, 7=Sun
START=$(date -d "-$((DOW - 1)) days" +%Y-%m-%d)
END=$(date +%Y-%m-%d)

# last_week
DOW=$(date +%u)
START=$(date -d "-$((DOW + 6)) days" +%Y-%m-%d)
END=$(date -d "-${DOW} days" +%Y-%m-%d)
...
## Bash Date Arithmetic (macOS `date -v`) — reference only

```bash
# macOS-only: BSD `date` uses `-v`; GNU/Linux uses `date -d` (see above). Kept for reference.
# today
START=$(date +%Y-%m-%d)
END=$(date +%Y-%m-%d)
