...
6. ~~**Claude Sessions**: check if `~/.claude/history.jsonl` exists~~ — disabled in opencode — history file is Claude-Code-specific
...
| Claude Sessions  | ✗ Disabled  | disabled in opencode — history file is Claude-Code-specific |
```
...
Determine the skill directory (resolve symlinks). Write `config.json` — see [config.example.json](config.example.json) for the schema.
...
```bash
SKILL_DIR="$HOME/.opencode/skills/brag"
if [[ -L "$SKILL_DIR" ]]; then
...
Compute START_DATE and END_DATE as `YYYY-MM-DD` strings using Linux `date -d` (macOS `date -v` equivalents kept in the reference for portability).
...
4. ~~**Claude Code Sessions** — read `~/.claude/history.jsonl`, deduplicate by sessionId~~ — disabled in opencode — history file is Claude-Code-specific
