...
If a feature needs a DB table, column, or RLS policy that does not exist → **STOP**. Do not script migration, do not edit `supabase/`, do not invent a data-layer workaround. Prepare the migration SQL or schema changes needed. Present the schema changes to the user and ask if you should apply them. Wait for confirmation before applying.
...
- **Phosphor icons**, **motion-primitives**, **Watermelon UI**, and **Skiper UI** are available for reference.
- Reminder: **Do not add as dependencies without user approval** per Lovable boundaries.
- See **`premium-frontend-system`** skill for details.
